...
Two-column flex (`main-container`): a **left config pane** (`400px`) and a **right game pane** (`flex:1`). Responsive: stacks vertically at ≤600px; left pane shrinks to 350px / fruit panel to 180px at ≤1200px.
...
│  (config)        │                                          │
│                  │  ┌──────────────────┐  VS  ┌────────────┐│
│  API URL         │  │🔴 P1 model/stats │      │🔵 P2 stats ││
│  API Key         │  │  latency graph   │      │  latency   ││
│  ⚡ Load Models  │  │  min/med/p90/max │      │  graph     ││
│  P1 model search │  └──────────────────┘      └────────────┘│
│  P2 model search │     ⏱️ MM:SS  timer                       │
│  ⚙️ Options ▶    │  ┌────────┬───────────────────┬────────┐ │
│   - Visibility   │  │ Fruit  │   Game Canvas     │ Game   │ │
│   - Coll. Avoid  │  │ Legend │   (600×600)       │ Log    │ │
│   - Debug        │  │ (left) │   30×30 grid      │ (right)│ │
│  ⚡ Extra ▶      │  │        │                   │ 600px  │ │
│   - Speed Test   │  └────────┴───────────────────┴────────┘ │
│   - Sort btns    │                                          │
│  [Start][⏸][🔄]  │                                          │
...
### Right pane (game)
- **Stats panel** (below the game layout, not above): two `.player-card`s separated by a `VS`. Each card: colored dot, model name (truncated >25 chars → 22+"…"), and metrics `length/moves/↑bytes ↓bytes`. Each card also contains a latency graph + stats block.
- **Timer** (`#game-timer`): `⏱️ M:SS`, sits at top of the game pane above the layout. Freezes at game end via `finalElapsedTime`.
- **Three-column game-layout** (flex, `gap:20px`):
  - **Left — Fruit Legend** (`.fruit-legend`, ~200px): always visible, 7 items with emoji/name/value/rarity.
  - **Center — Canvas** (`#game-canvas`, 600×600px, 30×30 grid at 20px/cell, bg `#0a0a15`, subtle grid, glowing teal border).
  - **Right — Game Log** (`.game-log-panel`, 250px wide, **fixed `height:600px`**, scrollable `#log-content`, 11px font, custom teal scrollbar).
