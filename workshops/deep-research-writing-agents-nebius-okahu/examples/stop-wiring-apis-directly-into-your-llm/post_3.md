...
1/ Host – the brain
LLMs handle reasoning, managing context for multi-step tasks.

2/ Client – the contractor
It connects to the **Global Tool Registry**, asking for tools by tag. This decouples API calls, no hardcoding.

3/ Servers – the hands
Each Server is a specialized microservice. It does one job (e.g., GitHub, CRM) and returns results.

Our PR Reviewer Assistant proves this. We don't wire Gemini directly to GitHub, Jira, or Slack.

Our Host (Gemini-powered) detects a new PR.
It asks the Client for tools. The **Global Tool Registry** routes requests to Servers.

Servers fetch PR details, Jira context, and the prompt. Gemini generates the review. A Server posts feedback to Slack.
